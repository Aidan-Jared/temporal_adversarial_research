...
def initialize_wandb(args: Namespace) -> None:
    ...
    mode = (
        "disabled"
        if os.getenv("MAMMOTH_TEST", "0") == "1"
        else os.getenv("WANDB_MODE", "online")
    )
    wandb.init(
...
def train_single_epoch(
    ...
    while True:
        ...
        inputs, labels = (
            ...
            labels.to(model.device, dtype=torch.long),
        )
        not_aug_inputs = not_aug_inputs.to(model.device)

        extra_fields = {
            train_loader.dataset.extra_return_fields[k]: _to_device(
                train_loader.dataset.extra_return_fields[k], data[3 + k], model.device
            )
            for k in range(len(data) - 3)
        }

        loss = model.meta_observe(
            inputs, labels, not_aug_inputs, epoch=epoch, **extra_fields
        )
        ...
        pbar.set_postfix(
            {"loss": loss, "lr": model.opt.param_groups[0]["lr"]}, refresh=False
        )
        pbar.update()
...
@can_save_and_exit
def train(
    model: ContinualModel, dataset: ContinualDataset, args: Optional[Namespace] = None
) -> None:
    """
    ...
    if args is None:
        assert "MAMMOTH_ARGS" in os.environ, (
            "No args provided, please set the MAMMOTH_ARGS environment variable"
        )
    ...
    with track_system_stats(logger, device=args.device) as system_tracker:
        ...
        if args.eval_future:
            assert isinstance(model, FutureModel), (
                "Model must be an instance of FutureModel to evaluate on future tasks"
            )
        ...
        for cur_task in range(start_task, end_task):
            ...
            if not issubclass(dataset.__class__, GCLDataset):
                assert issubclass(
                    train_loader.dataset.__class__, MammothDatasetWrapper
                ), (
                    "Dataset must be an instance of MammothDatasetWrapper (did you forget to call the `store_masked_loaders`?)"
                )

            if (
                can_compute_fwd_beforetask
                and is_fwd_enabled
                and args.enable_other_metrics
            ):
                # try to compute accuracy at the beginning of the task
                try:
                    logging.info(
                        "Evaluating model before task (for Forward Transfer metric)..."
                    )
                    random_res_class, random_res_task = dataset.evaluate(
                        model, dataset, last=True
                    )  # the ugliness of this line is for backward compatibility
                    random_results_class.append(random_res_class)
                    random_results_task.append(random_res_task)
                except Exception:
                    logging.info(
                        "Could not evaluate before `begin_task`, will try after"
                    )
            ...
            if (
                not can_compute_fwd_beforetask
                and is_fwd_enabled
                and args.enable_other_metrics
            ):
                if (
                    train_loader.dataset.num_times_iterated == 0
                ):  # compute only if the model has not been trained yet
                    try:
                        logging.info(
                            "Evaluating model before task (for Forward Transfer metric)..."
                        )
                        ...
                        logging.error(
                            f"Model `{model.NAME}` does not support pre-evaluation, will not compute Forward Transfer metric\n{e}"
                        )
                        is_fwd_enabled = False
                else:
                    logging.info(
                        "Model used the training data, skipping Forward Transfer metric compute"
                    )
                    is_fwd_enabled = False
            ...
            if not args.inference_only and args.n_epochs > 0:
                if cur_task and args.enable_other_metrics:
                    ...
                    if dataset.SETTING == "class-il":
                        results_mask_classes[cur_task - 1] = (
                            results_mask_classes[cur_task - 1] + accs[1]
                        )
                ...
                scheduler = (
                    get_scheduler(model, args, reload_optim=True)
                    if not hasattr(model, "scheduler")
                    else model.custom_scheduler
                )
                ...
                if not isinstance(dataset, GCLDataset):
                    n_iterations = (
                        model.args.n_epochs * len(train_loader)
                        if model.args.fitting_mode == "epochs"
                        else model.args.n_iters
                    )
                mininterval = (
                    0.2 if n_iterations is not None and n_iterations > 1000 else 0.1
                )
                train_pbar = tqdm(
                    train_loader,
                    total=n_iterations,  # train_loader is actually ignored, will update the progress bar manually
                ...
                if args.non_verbose:
                    logging.info(
                        f"Task {cur_task + 1}"
                    )  # at least print the task number
                ...
                while True:
                    ...
                    if args.fitting_mode == "epochs" and epoch >= model.args.n_epochs:
                        ...
                        and model.task_iteration >= model.args.n_iters
                    ):
                        break
                    elif (
                        args.fitting_mode == "early_stopping"
                        and epoch % args.early_stopping_freq == 0
                        and epoch > 0
                    ):
                        epoch_accs, _, epoch_loss = eval_dataset.evaluate(
                            model, eval_dataset, return_loss=True, last=True
                        )
                        ...
                        if args.early_stopping_metric == "accuracy":
                            ...
                            raise ValueError(
                                f"Unknown early stopping metric {args.early_stopping_metric}"
                            )

                        # Higher accuracy is better
                        if (
                            best_ea_metric is not None
                            and ea_metric - best_ea_metric < args.early_stopping_epsilon
                        ):
                            ...
                            if cur_stopping_patience <= 0:
                                ...
                                model.load_state_dict(
                                    {
                                        k: v.to(model.device)
                                        for k, v in best_ea_model.items()
                                    }
                                )
                                break
                            logging.info(
                            ...
                            best_ea_model = copy.deepcopy(
                                {k: v.cpu() for k, v in model.state_dict().items()}
                            )
                            cur_stopping_patience = args.early_stopping_patience

                    if (
                        args.eval_epochs is not None
                        and (epoch > 0 or args.eval_epochs)
                        and epoch % args.eval_epochs == 0
                        and epoch < model.args.n_epochs
                    ):
        ...
        if args.validation:
            ...
            final_dataset.log(
                args, logger, accs, "final", final_dataset.SETTING, prefix="FINAL"
            )
        ...
        if args.enable_other_metrics:
            ...
            forgetting, forgetting_mask_class = logger.add_forgetting(
                results, results_mask_classes
            )
            log_extra_metrics(
                args, forgetting, forgetting_mask_class, "Forgetting", cur_task
            )
            if is_fwd_enabled:
                fwt, fwt_mask_class = logger.add_fwt(
                    results,
                ...
                log_extra_metrics(
                    args, fwt, fwt_mask_class, "Forward Transfer", cur_task
                )
            else:
                logging.warning(
                    "Forward Transfer metric incompatible with the current model, skipped."
                )
